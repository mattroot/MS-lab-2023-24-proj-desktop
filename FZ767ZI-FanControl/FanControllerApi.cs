...
namespace FZ767ZI_FanControl
{
    public class FanControllerApi
    {
        ...
        public bool IsCalibratingFirstStep { get; protected set; }
        public bool Fan1Calibrating = false;
        public bool Fan2Calibrating = false;
        public bool Fan1AwaitingResults = false;
        public bool Fan2AwaitingResults = false;
                                ...
                                if (item.StartsWith("ACK,GCL,fan1,")) { 
                                 // 13
                                    ...
                                    if (Fan1CalibrationChanged != null)
                                        Fan1CalibrationChanged(this, new CalibrationResultsEventArgs(cr.Item1, cr.Item2));
                            ...
                            if (Fan2AwaitingResults)
                            {
                                if (item.StartsWith("ACK,GCL,fan2,"))
                                {
                                    ...
                                    if (Fan2CalibrationChanged != null)
                                        Fan2CalibrationChanged(this, new CalibrationResultsEventArgs(cr.Item1, cr.Item2));
                                    Fan2AwaitingResults = false;
                                    if (!Fan1AwaitingResults) IsCalibratingFirstStep = false;
                                }
                            }
                        }
                    } catch (Exception x) {
        ...
        protected Tuple<int, int> ParseCalibrationResult(string result) {
        ...
        protected FanDetails ParseDetails(string details) {
            ...
            fd.CurrentSpeed = int.Parse(det[1].Substring(0, det[1].IndexOf('.')));
            fd.DutyCycle = int.Parse(det[2].Substring(0, det[2].IndexOf('.')));
            fd.TargetSpeed = int.Parse(det[3].Substring(0, det[3].IndexOf('.')));
        ...
        public void SetMode(string fanName, PWM_FAN_CTRL_MODE mode, int value) {
            ...
            string query = "SET," + fanName + "," + (int)mode + "," + value;
            WriteASCII(query);
        }
